...
cover_images_path = "./output"
target_images_path = "./input"
...
def merge_face(cover, target, rect, mask):
    [x, y, w, h] = rect
...
cover_images_path_list = get_image_paths(cover_images_path)
...
patch_rects = []
patch_rects.append([0, 420, 1080, 1080, 1, 99999999])
...
for i in range(start_index, len(cover_images_path_list)):
    print(f"frame:{i+1}")
    # print(f"cover image:{cover_images_path_list[i]}")
    # print(f"target image:{target_images_path_list[i]}")

    cover_img = None
    target_img = None
    output_filename = os.path.basename(cover_images_path_list[i])
    ...
    for patch_index, patch_rect in enumerate(patch_rects):
        ...
        if i + 1 >= start_frame and i + 1 <= end_frame:
            if cover_img == None:
                cover_img = Image.open(cover_images_path_list[i])
                target_img = Image.open(target_images_path_list[i])

                cover_img_arr = np.array(cover_img)
                target_img_arr = np.array(target_img)

                if cover_img.width != frame_width or cover_img.height != frame_height:
                    fit_height = (int)(cover_img.height * (frame_width / cover_img.width))
                    if fit_height != cover_img.height:
                        cover_img_arr = resize_img(cover_img_arr, frame_width, fit_height)
                    if frame_height > fit_height:
                        top = (int)((frame_height - fit_height) / 2) + mod_y
                        bottom = (frame_height - fit_height) - top
                        cover_img_arr = np.pad(cover_img_arr, pad_width=((top, bottom), (0, 0), (0, 0)), mode="constant")
                    elif fit_height > frame_height:
                        top = (int)((fit_height - frame_height) / 2)
                        bottom = (fit_height - frame_height) - top
                        cover_img_arr = cover_img_arr[top:frame_height, 0:frame_width]
                    output_cover_filename = f"{os.path.splitext(output_filename)[0]}-cover.png"
                    Image.fromarray(cover_img_arr).save(os.path.join(output_mask_folder, output_cover_filename))

                if target_img.width != frame_width or target_img.height != frame_height:
                    target_img_arr = resize_img(target_img_arr, frame_width, frame_height)
    ...
    for patch_index, (patch_coord, masks) in enumerate(zip(patch_coords, masks)):
        [x, y, w, h] = patch_coord
        target_img_arr = merge_face(cover_img_arr, target_img_arr, patch_coord, mask)
