# modified from https://github.com/s9roll7/ebsynth_utility
...
def ebsynth_utility_stage5(dbg, project_args, is_invert_mask):
    ...
    if not os.path.isdir(img2img_upscale_key_path):
        if not os.path.isdir(img2img_key_path):
            dbg.print("img2img_key_path : no such dir %s" % img2img_key_path)
            return
        ...
        if img_height1 != img_height2 or img_width1 != img_width2:
            print(f"The size of frame and img2img_key NOT matched. ({img_width1}x{img_height1} != {img_width2}x{img_height2})")
            return
...
if __name__ == "__main__":
    print("# make ebsynth project file")
    print("python run_stage5.py [Input frames path] [Keyframe save path]")
    print("ex) python run_stage5.py ./input ./key")
    dbg = debug_string()
    argument = sys.argv
    if len(argument) > 2:
        ebsynth_utility_stage5(dbg, ("./", "", argument[1], "", "", argument[2], ""), False)
