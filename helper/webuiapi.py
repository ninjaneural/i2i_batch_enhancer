...
class ControlNetUnit:
    ...
    def to_dict(self):
        ...
        if self.threshold_a == -1:
            del dict["threshold_a"]
        if self.threshold_b == -1:
            del dict["threshold_b"]
        return dict


def b64_img(image: str) -> str:
    return "data:image/png;base64," + raw_b64_img(image)


def raw_b64_img(image: str) -> str:
...
class WebUIApi:
    ...
    def txt2img(
        ...
        if use_deprecated_controlnet and controlnet_units and len(controlnet_units) > 0:
            ...
            return self.custom_post("controlnet/txt2img", payload=payload, use_async=use_async)

        if controlnet_units and len(controlnet_units) > 0:
            payload["alwayson_scripts"]["ControlNet"] = {"args": [x.to_dict() for x in controlnet_units]}
        elif self.has_controlnet:
            # workaround : if not passed, webui will use previous args!
            payload["alwayson_scripts"]["ControlNet"] = {"args": []}

        return self.post_and_get_api_result(f"{self.baseurl}/txt2img", payload, use_async)
    ...
    def img2img(
        ...
        if use_deprecated_controlnet and controlnet_units and len(controlnet_units) > 0:
            payload["controlnet_units"] = [x.to_dict() for x in controlnet_units]
            return self.custom_post("controlnet/img2img", payload=payload, use_async=use_async)
        ...
        return self.post_and_get_api_result(f"{self.baseurl}/img2img", payload, use_async)
    ...
    def extra_single_image(
        ...
        return self.post_and_get_api_result(f"{self.baseurl}/extra-single-image", payload, use_async)
    ...
    def extra_batch_images(
        ...
        return self.post_and_get_api_result(f"{self.baseurl}/extra-batch-images", payload, use_async)
    ...
    def interrogate(self, image, model):
        payload = {"image": b64_img(image), "model": model}
